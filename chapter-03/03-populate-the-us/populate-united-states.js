...
const combineWith = (storage, key) => (members) =>
  storage[key] ? [...members, ...JSON.parse(storage[key])] : members;
const save = (storage, key) => (data) => {
  storage[key] = JSON.stringify(data);
  return data;
};
const logFieldsToConsole =
  (field, delimiter = ",") =>
  (members) => {
...
const printStatesForMapping = (stateCounts) =>
  Object.keys(stateCounts).reduce((obj, s) => {
